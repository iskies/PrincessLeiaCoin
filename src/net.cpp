...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Princessleiacoin " + FormatFullVersion();
        ...
        if (HaveNameProxy()) {
            ...
            if (LookupHost(strDNSSeed[seed_idx][1], vaddr))
            {
                BOOST_FOREACH(CNetAddr& ip, vaddr)
                {
                    ...
                    addr.nTime = GetTime() - 1*nOneDay - GetRand(4*nOneDay); // use a random age between 3 and 7 days old
                    vAdd.push_back(addr);
                    found++;
                }
            }
...
unsigned int pnSeed[] =
{
    0x8f8e448a,0x8a448e8f,0x352c3eb2,0xb23e2c35
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Princessleiacoin is probably already running."), addrBind.ToString().c_str());
        else
