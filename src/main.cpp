...
#if defined(NDEBUG)
# error "Princessleiacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x8aea02cc31868ce56dbc083a832276838ecc8a6e1711401d69a6267a1870f690");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Princessleiacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Princessleiacoin Signed Message:\n";
...
{
    // Princessleiacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 900000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 900000); // Princessleiacoin: 

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Princessleiacoin: 1 days
static const int64 nTargetSpacing = 2 * 60; // Princessleiacoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Princessleiacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Princessleiacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xfde8d8fa25ffad1ded41fc11cd80d0e407a3d2a164bbe9df0da41d73e76d4fac");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Elon Musk overtakes Jeff Bezos as worlds richest man";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 2500 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04ebe5d94f40ebe97a7040e7a65629b39e0b710ccb4af79d5c9000e0204d09bbcc9f138e26a617aa3fdf46f22d038a593c86fd4d99e29e7a73669d6012c6c1a1a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1615902363;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086084420;

        if (fTestNet)
        {
            block.nTime    = 1615902350;
            block.nNonce   = 386143972;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x5c617a3be20d22efa0a52f992fce13403e92cea023604e4bdae99a2e5498e3a1"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xca, 0xb4, 0xdc }; // Princessleiacoin
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PrincessleiacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PrincessleiacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PrincessleiacoinMiner(CWallet *pwallet)
{
    printf("PrincessleiacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("princessleiacoin-miner");
    ...
    try { loop {
        ...
        printf("Running PrincessleiacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PrincessleiacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PrincessleiacoinMiner, pwallet));
}
